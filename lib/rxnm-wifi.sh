...
action_hotplug() {
    ...
    if ! is_service_active "systemd-networkd"; then
        ...
        if [ "$is_usb_gadget" = true ]; then
            if type configure_standalone_gadget >/dev/null 2>&1; then
                configure_standalone_gadget "$iface"
            else
                log_warn "Rescue gadget helper not found."
            fi
        else
            if type configure_standalone_client >/dev/null 2>&1; then
...
action_delete_netdev() {
    ...
    local found="false"
    # Clean up all related files via loop instead of glob check inside loop
...
action_set_static() {
    ...
    if [ -z "$iface" ] || [ -z "$ip" ]; then
        log_error "Interface and IP required"
        return 1
    fi
