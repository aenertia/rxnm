...
if [ "$WIFI_ONLY" = "false" ]; then
    ...
    CONVERGED=false
    IP=$(wait_ip_convergence "$CLIENT" "host0" "192.168.213.60" "inet" 10)
    if [ -n "$IP" ]; then
    ...
    CONVERGED=false
    CLI_IP=$(wait_ip_convergence "$CLIENT" "host0" "fd00:cafe::3" "inet6" 8)
    SRV_IP=$(wait_ip_convergence "$SERVER" "host0" "fd00:cafe::2" "inet6" 8)
